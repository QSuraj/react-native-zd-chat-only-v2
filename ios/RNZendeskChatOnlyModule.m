...
#import "RNZendeskChatOnlyModule.h"
...
@interface RNZendeskChatOnlyModule ()
@end

@implementation RNZendeskChatOnlyModule
// Backwards compatibility with the unnecessary setVisitorInfo method
ZDKChatAPIConfiguration *_visitorAPIConfig;


RCT_EXPORT_MODULE(RNZendeskChatOnlyModule);
...
- (ZDKChatAPIConfiguration*)applyVisitorInfo:(NSDictionary*)options intoConfig:(ZDKChatAPIConfiguration*)config {
	...
	NSLog(@"[RNZendeskChatOnlyModule] Applied visitor info: department: %@ tags: %@, email: %@, name: %@, phone: %@", config.department, config.tags, config.visitorInfo.email, config.visitorInfo.name, config.visitorInfo.phoneNumber);
	return config;
}

#define RNZDKConfigHashErrorLog(options, what)\
if (!!options) {\
	NSLog(@"[RNZendeskChatOnlyModule] Invalid %@ -- expected a config hash", what);\
}
...
RCT_EXPORT_METHOD(startChat:(NSDictionary *)options) {
	if (!options || ![options isKindOfClass: NSDictionary.class]) {
		if (!!options){
			NSLog(@"[RNZendeskChatOnlyModule] Invalid JS startChat Configuration Options -- expected a config hash");
		}
	...
	dispatch_sync(dispatch_get_main_queue(), ^{
		...
		if (!!error) {
			NSLog(@"[RNZendeskChatOnlyModule] Internal Error loading ZDKChatEngine %@", error);
			return;
		}
		...
		UIViewController *viewController = [ZDKMessaging.instance buildUIWithEngines:engines
																 configs:@[chatConfig, messagingConfig]
																   error:&error];
		if (!!error) {
			NSLog(@"[RNZendeskChatOnlyModule] Internal Error building ZDKMessagingUI %@",error);
			return;
		}
